...
const Header = () => {
  const verifyToken = () => {
    if (localStorage.getItem("token") != undefined) {
      Axios.post("http://localhost:8080/http://localhost:5000/verifyToken", {
          ...
          if (result.data.status == "valid") {
            if (window.outerWidth < 450) {
              ...
              <div id="loggedIn">
                ...
                <span class="userButton">${localStorage.getItem(
                  "userName"
                )}</span>
              </div>
            `
              );
              document.getElementById("loginPhoneButton").style.display =
                "none";
              document.getElementById("logo").style.display = "block";
            } else {
              document.getElementById("login").insertAdjacentHTML(
                "afterend",
                `
            <div id="loggedIn">
              <img class="userIcon" src = ${userImage}/>
  ...
  return (
    <div id="header">
      ...
      <button className="btn btn-success" type="button" id="login">
        Login
      </button>
